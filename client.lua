	...
	if isDead and spam then
		ESX.TriggerServerCallback('hhfw:docOnline', function(EMSOnline, hasEnoughMoney)
			...
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				...
				Notify("Le médecin arrive")
			else
				if EMSOnline > Config.Doctor then
					Notify("Trop de médecins en service", "error")
				elseif not hasEnoughMoney then
					Notify("Pas assez d'argent")
				else
					Notify("Le médecin est sur la route")
				end	
			end
		end)
	else
		Notify("Il faut être dans le coma !")
	end
end)
...
function DoctorNPC()
	...
	Notify("Votre traitement est terminer, vous avez payé : "..Config.Price)
